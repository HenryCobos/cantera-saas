...
export default function EditarViajePage() {
  ...
  const loadData = async () => {
    try {
      ...
      const currentCanteraId = (canteras?.[0] as { id: string } | undefined)?.id;
      if (!currentCanteraId) {
      ...
      // Cargar datos en paralelo: viaje existente, camiones, choferes, ventas
      const [viajeResult, camionesResult, choferesResult, ventasResult] = await Promise.all([
        supabase
          .from('viajes')
          .select('*')
          .eq('id', id)
      ...
      if (viajeResult.error) throw viajeResult.error;
      ...
      const viaje = (viajeResult as any).data;
      if (viaje) {
        setFormData({
          cantera_id: viaje.cantera_id || currentCanteraId,
          camion_id: viaje.camion_id || '',
          chofer_id: viaje.chofer_id || '',
          venta_id: viaje.venta_id || '',
          fecha: viaje.fecha ? new Date(viaje.fecha).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
          cantidad_metros: viaje.cantidad_metros?.toString() || '',
          costo_combustible: viaje.costo_combustible?.toString() || '',
          costo_peaje: viaje.costo_peaje?.toString() || '',
          otros_costos: viaje.otros_costos?.toString() || '',
          destino: viaje.destino || '',
        });
      }

      setCamiones((camionesResult.data || []) as any[]);
      setChoferes((choferesResult.data || []) as any[]);
      ...
      <div className="p-6">
        ...
        <div className="mx-auto max-w-2xl rounded-lg bg-white shadow">
          <div className="border-b border-gray-200 px-6 py-4">
            <h2 className="text-lg font-medium text-gray-900">Editar Viaje</h2>
            <p className="mt-1 text-sm text-gray-500">Modifica la información del viaje</p>
          </div>
